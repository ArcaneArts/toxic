# 1.3.9
* Fix `Pool.take` growth behavior causing immediate empty-pool failures
* Fix `Multiplexer` buffer initialization crash on first input event
* Fix `Stream.firstOrNull` and `Stream.lastOrNull` on single-subscription streams
* Fix `Stream.parallelMap` dropping results due to early close behavior
* Fix `Stream.accumulateBy` duplicate emissions for overweight chunks
* Fix `Iterable.reversedList` returning unreversed lists
* Fix `Random.nextString` to use the receiver RNG and guard empty charset
* Fix `num.readableFileSize` unit-boundary formatting behavior
